...
class ALEScreen { 
  public:
    ...
    /** Access a whole row */
    pixel_t *getRow(int r);
    
    /** Access the whole array */
    pixel_t *getArray() { return m_pixels.data(); }
    const pixel_t *getArray() const { return m_pixels.data(); }
...
inline bool ALEScreen::equals(const ALEScreen &rhs) const {
  return (m_rows == rhs.m_rows &&
          m_columns == rhs.m_columns &&
          (memcmp(m_pixels.data(), rhs.m_pixels.data(), arraySize()) == 0) );
}
...
inline pixel_t ALEScreen::get(int r, int c) const {
  ...
  return m_pixels.data()[r * m_columns + c];
}
...
// Access a whole row
inline pixel_t* ALEScreen::getRow(int r) {
  assert (r >= 0 && r < m_rows);
  return m_pixels.data() + r * m_columns;
}
