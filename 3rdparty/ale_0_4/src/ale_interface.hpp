...
public:
    ...
    void loadROM(string rom_file) {
        int argc = 6;
        const char* argv[6] = {"./ale", "-player_agent", "random_agent", "-display_screen", display_active ? "true" : "false", rom_file.c_str()};

        createOSystem(argc, const_cast<char**>(argv), theOSystem, theSettings);
